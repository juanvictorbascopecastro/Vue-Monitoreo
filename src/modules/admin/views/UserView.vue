<template>
  <div>
    <div class="sm:flex sm:justify-between sm:items-center">
      <h1 class="text-2xl font-bold">Registro de usuarios</h1>
      <!-- Right: Actions -->
      <div
    ...
    <div class="flex w-full mt-4">
      <table class="w-full table-auto text-left">
        ...
        <tbody>
          <template v-for="(item, i) in $store.state.user.list" :key="i">
            <tr>
        ...
        <tbody v-if="$store.state.user.list.length <= 0">
          <tr v-if="$store.state.user.isLoading">
            <th colspan="7">
              <div
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      if (store.state.user.list.length <= 0) store.dispatch("user/load", null);
    });
