...
import { CIUDAD_COLLECTION } from "../../../../common/collections";
...
export const load = async ({ commit }) => {
  try {
    ...
    const querySnap = await getDocs(
      query(collection(db, CIUDAD_COLLECTION), orderBy("name", "asc"))
    );
    const list = [];
...
export const create = async ({ commit, dispatch }, values) => {
  try {
    ...
    const docRef = await addDoc(collection(db, CIUDAD_COLLECTION), values);
    await dispatch("load");
...
export const update = async ({ commit, dispatch }, values) => {
  try {
    ...
    const docRef = await setDoc(doc(db, CIUDAD_COLLECTION, code), data);
    // console.log(docRef);
    await dispatch("load");
...
export const remove = async ({ commit, dispatch }, code) => {
  try {
    ...
    await deleteDoc(doc(db, CIUDAD_COLLECTION, code));
    // await dispatch("load");
